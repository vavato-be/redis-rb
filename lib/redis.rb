...
class Redis
  ...
  # @param opts          [Hash]          several options for `XCLAIM` command
  #
  # @option opts [Integer] :count      number of messages to claim (default 1)
  # @option opts [Boolean] :justid     whether to fetch just an array of entry ids or not.
  #                                    Does not increment retry count when true
  #
  # @return [Hash{String => Hash}] the entries successfully claimed
  # @return [Array<String>]        the entry ids successfully claimed if justid option is `true`
  def xautoclaim(key, group, consumer, min_idle_time, start, **opts)
    args = [:xautoclaim, key, group, consumer, min_idle_time, start]
    args.concat(['COUNT', opts[:count]])           if opts[:count]
    args << 'JUSTID'                               if opts[:justid]
    blk = opts[:justid] ? HashifyStreamAutoclaimJustId : HashifyStreamAutoclaim
    synchronize { |client| client.call(args, &blk) }
  end
